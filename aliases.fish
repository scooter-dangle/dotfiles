...
function ag \
	...
	gogogo $matching_lines \
	| fish_indent \
	| pygmenter \
	| highlight $target
end

function line_number_matches \
	--argument-names target target_file
...
# helper
function pp_fish_aliases \
	--argument-names target
	sed -ue "s/\([^)]\);\s*/\1\n/g" $target \
	| fish_indent
end
# helper
function prettify_aliases --description "Prettifies contents of aliases.fish"
	pp_fish_aliases $aliases >  $aliases
end
# helper
# Still under construction
function uglify_aliases --description "Uglifies contents of aliases.fish"
end
# helper
function pygmenter --description "Runs argument through pygmentize -lbash only if pygmentize can be found" \
	--argument-names target
...
function qcount --description "Quiet count. Use exit status without printing"
	count $argv >  /dev/null
end
...
function where --description "Get the directory of an executable" \
	--argument-names target
